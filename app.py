import streamlit as st # Mengimpor library Streamlit untuk antarmuka web
import pandas as pd # Mengimpor Pandas untuk manipulasi data
import numpy as np # Mengimpor Numpy untuk operasi numerik
import plotly.express as px # Mengimpor Plotly Express untuk grafik interaktif
import plotly.graph_objects as go # Mengimpor Plotly Graph Objects untuk grafik kustom
import os # Mengimpor OS untuk interaksi dengan sistem file
import joblib # Mengimpor Joblib untuk memuat model Machine Learning

# Konfigurasi awal halaman web
st.set_page_config(
    page_title="Global Carbon Dashboard", # Judul halaman pada tab browser
    layout="wide", # Menggunakan layout lebar penuh
    initial_sidebar_state="collapsed" # Menyembunyikan sidebar secara bawaan
)

# Inisialisasi state untuk menyimpan data sesi pengguna agar tidak hilang saat reload
...
# Menyuntikkan kode CSS kustom untuk desain tema "Gen Z Retro Roundy"
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@500;700;800&display=swap');

/* Aturan font dan warna teks utama */
...
[data-testid="stSidebar"] { display: none; } /* Menyembunyikan sidebar bawaan */
.block-container { padding: 2rem 2.25rem 5rem !important; max-width: 1250px !important; }

/* Desain kartu bergaya retro dengan border tebal dan shadow */
.retro-card {
...
.retro-card:hover { transform: translateY(-3px); box-shadow: 8px 8px 0px #022C22 !important; }

/* Desain tombol aksi dan navigasi */
.stButton > button {
    background-color: #FFFFFF !important;
...
/* Modifikasi input dropdown, selectbox, multiselect agar background putih */
...
div[data-baseweb="select"] * { color: #022C22 !important; font-weight: 700 !important; }

/* Background dropdown list dibuat putih sesuai permintaan */
div[role="listbox"], [data-baseweb="menu"], [data-baseweb="popover"], div[data-baseweb="popover"] * {
    background-color: #FFFFFF !important;
}
ul[role="listbox"] {
    background-color: #FFFFFF !important;
...
ul[role="listbox"] li { background-color: #FFFFFF !important; color: #022C22 !important; font-weight: 700 !important; }
...
# Membuat 3 kolom untuk navigasi tab custom
nav_1, nav_2, nav_3 = st.columns(3)
with nav_1:
    if st.button("PAPAN DATA", use_container_width=True): # Tombol tab Dashboard
        st.session_state.page = "dashboard"
        st.rerun()
with nav_2:
    if st.button("COBA SIMULASI", use_container_width=True): # Tombol tab Simulator
        st.session_state.page = "simulator"
        st.rerun()
with nav_3:
    if st.button("ATURAN BARU", use_container_width=True): # Tombol tab Kebijakan
        st.session_state.page = "kebijakan"
        st.rerun()

# Fungsi untuk memuat dataset dengan cache agar performa lebih cepat
@st.cache_data
def load_data():
    paths = [
    ...
    # Coba memuat file CSV dari daftar jalur yang ada
    for p in paths:
        if os.path.exists(p):
            df = pd.read_csv(p)
    ...
    # Data Dummy digunakan jika semua file CSV tidak ditemukan
    np.random.seed(42)
    ...
    for c in countries:
        ...
        for y in range(2000, 2026):
            ...
            rows.append({
                "Country": c, "Year": y, "Forest_Area_km2": max(fa, 5000),
...
# Fungsi untuk memuat model ML (XGBoost)
@st.cache_resource
def load_ml_model():
...
# Fungsi untuk memprediksi karbon, menggunakan ML jika tersedia, rumus matematis jika tidak
...
# Memuat data utama ke dataframe
df = load_data()
...
# Konfigurasi gaya grafik Plotly agar seragam dengan tema CSS
CHART_LAYOUT = dict(
...
# Fungsi untuk memfilter data berdasarkan input dari dashboard
...
if page == "dashboard":
    ...
    # Visualisasi Bar Chart Faktor Penyebab & Negara Tertinggi
...
# ==================== HALAMAN 2: SIMULATOR ====================
    ...
    with st.form("form_sim"):
        ...
        # Input umum yang terlihat langsung
        ...
        # Input lanjutan disembunyikan dalam expander
        with st.expander("⚙️ PENGATURAN LANJUTAN (ADVANCE)"):
            st.write("Sesuaikan parameter lanjutan dengan menginput nilai secara langsung atau menggeser slider.")
            # Menarik data default dari dataset asli berdasarkan negara yang dipilih
            ...
            a1, a2 = st.columns(2)
            luas_h_input = a1.number_input("Luas Hutan Awal (Forest_Area_km2)", min_value=0.0, value=def_forest, step=10000.0)
            luas_l_input = a2.number_input("Luas Total Daratan (Land_Area_km2)", min_value=0.0, value=def_land, step=10000.0)
    ...
    if run_btn:
        ...
        # Iterasi prediksi berdasarkan tahun
        ...
        # Analisis hasil persentase perubahan
...
# ==================== HALAMAN 3: KEBIJAKAN ====================
    ...
    with col_l:
        ...
        # Pilihan negara untuk kebijakan
    ...
    with col_r:
        # Menarik data land area, forest area, dan laju pertumbuhan riil sesuai dataset
        ...
        # Mengambil base rate secara dinamis
        ...
        # Pengaruh modifikasi kebijakan
        if p1: bd *= 0.3
        if p2: ba *= 3.0
        if p3: bd *= 0.8
        if p4: ba *= 1.4; bd *= 0.9

        # Prediksi dampak ke tahun 2030
