...
# ─────────────────────────────────────────────────────────────
# TEMA GEN Z RETRO ROUNDY (DARK GREEN & WHITE)
# ─────────────────────────────────────────────────────────────
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@500;700;800&display=swap');

/* GLOBAL SETTINGS - Mengganti semua elemen menjadi Dark Green (#022C22) */
* {
...
body, .stApp, .block-container, header[data-testid="stHeader"] {
    background-color: #FAFAF9 !important; /* Putih hangat / soft cream */
}
...
/* ── CONTAINER Bawaan Streamlit (Pengganti HTML Div agar isi tidak bocor) ── */
...
.stButton > button:hover, .stButton > button:active {
    background-color: #D1FAE5 !important; /* Soft Green Hover */
...
/* ── FIX: Latar Belakang List Dropdown & Opsi ── */
...
/* ── TEKS KUSTOM HTML ── */
.title-text { font-size: 2.5rem; font-weight: 800; margin-bottom: 0.2rem; color: #022C22; }
    ...
    for c in countries:
        ...
        for y in range(2000, 2026):
            ...
            rows.append({
                ...
                "Primary_Driver_of_Change": np.random.choice(["Pertanian", "Penebangan", "Kebakaran", "Tambang"]),
                "Region": region(c),
            })
...
# ═══════════════════════ HALAMAN 2: SIMULATOR ══════════════════════════
    ...
    with st.container(border=True):
        ...
        st.markdown("<div class='sub-text'>Atur konfigurasi di bawah untuk memprediksi sisa cadangan karbon global.</div>", unsafe_allow_html=True)
    ...
    with st.form("form_sim"):
        with st.container(border=True):
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            with st.expander("PENGATURAN LANJUTAN (ADVANCE)"):
                # Menarik data land area dasar dari dataframe sebagai default
                base_data = df[df["Country"] == negara]
                def_land = int(base_data["Land_Area_km2"].values[0]) if not base_data.empty else 400000
                def_forest = int(base_data[base_data["Year"] == YEAR_MAX]["Forest_Area_km2"].values[0]) if not base_data.empty else 250000
                
                a1, a2 = st.columns(2)
                luas_h = a1.number_input("Luas Hutan Awal (km²)", value=def_forest, step=10000)
                luas_l = a2.number_input("Luas Total Daratan (km²)", value=def_land, step=10000)
                sken_iklim = st.selectbox("Skenario Iklim", ["Moderat", "Optimis (Cepat Tumbuh)", "Pesimis (Kering)"])
    ...
    if run_btn:
        hasil_list, thn_list = [], []
        # Mengaplikasikan pengaruh iklim terhadap pertumbuhan/penyusutan hutan
        faktor_iklim = 1.02 if "Optimis" in sken_iklim else (0.97 if "Pesimis" in sken_iklim else 1.0)

        for y in range(YEAR_MAX, thn_target + 1):
            fa = luas_h * (1 + (laju_a - laju_d) / 100) ** (y - YEAR_MAX)
            fa = fa * faktor_iklim # Terapkan faktor advance
        ...
        with st.container(border=True):
            st.markdown("<div class='header-text'>Prediksi Cadangan Karbon</div>", unsafe_allow_html=True)
...
# ═══════════════════════ HALAMAN 3: KEBIJAKAN ══════════════════════════
    ...
    with st.container(border=True):
        ...
        st.markdown("<div class='sub-text'>Coba terapkan kebijakan dan lihat dampaknya di tahun 2030.</div>", unsafe_allow_html=True)
    ...
    with col_r:
        ...
        with st.container(border=True):
            st.markdown("<div class='header-text'>Proyeksi di Tahun 2030</div>", unsafe_allow_html=True)
            m1, m2 = st.columns(2)
